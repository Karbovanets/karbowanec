...
Transaction buildPqTransaction(const std::vector<PqSpendInput>& inputs,
  ...
  // output's out_context (consensus recomputes it identically).
  std::vector<CryptoPQ::InputRef> refs(inputs.size());
  uint64_t sumIn = 0;
  for (size_t i = 0; i < inputs.size(); ++i) {
    std::memcpy(refs[i].prevTxid.data(), inputs[i].prevTxid.data, 32);
    refs[i].prevOutIndex = inputs[i].prevOutIndex;
    if (sumIn + inputs[i].amount < sumIn) {
      throw std::runtime_error("buildPqTransaction: input amount overflow");
    }
    sumIn += inputs[i].amount;
  }
